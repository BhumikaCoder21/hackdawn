...
return ( <header className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-[#2E8B57] via-[#57C84D] to-[#A8E063] text-white shadow-md"> <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
    ...
    <nav className="hidden md:flex gap-8 items-center font-[Poppins]">
      ...
      <button onClick={() => navigate("/postride")}>
        Post Ride
      </button>
  ...
  {menuOpen && (
    <div className="md:hidden bg-green-600 flex flex-col items-center gap-4 py-4">
      ...
      <button onClick={() => {navigate("/post"); setMenuOpen(false);}}>
        Post Produce
      </button>

      <button onClick={() => {navigate("/postride"); setMenuOpen(false);}}>
        Post Ride
      </button>

      <button onClick={() => {navigate("/trucks"); setMenuOpen(false);}}>
        Truck Routes
      </button>
